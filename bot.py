...
# РЕАКЦИИ: user_id -> [[post_key, +%, истекает_ts], ...] — активные бонусы к шансу;
# post_key ("chat:msg") -> [user_id, ...] — кто уже получил бонус за этот пост
# (один бонус на человека за пост — навсегда, чтобы нельзя было фармить повторно).
react_bonuses: Dict[int, List[List]] = {}
react_awarded: Dict[str, List[int]] = {}
_react_save_scheduled = False
_react_last_error = ""
_react_linked_cache: Dict[int, Tuple[float, Optional[int]]] = {}
_react_announce_times: List[float] = []
REACT_MAX_POSTS_KEPT = 300
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data in ("react_bonus", "react_hours", "react_max", "react_label"):
        ...
        prompts = {
            ...
            "react_label": (
                ...
                "Применится к новым постам; у уже опубликованных останется старое название."
            ),
        }
        await query.edit_message_text(
            prompts[data] + "\n\n❌ /cancel — отменить.",
    ...
    if data == "react_preview":
        ...
        await query.answer("Так сообщение увидят в чате (значения — примерные)")
        return
    ...
    if data == "react_reset":
        ...
        await query.answer("Стандартный текст возвращён")
        await show_react_menu(query)
        return
    ...
    for ch in targets:
        try:
            await context.bot.copy_message(
                ...
                reply_markup=react_wrap_markup(_post_markup(btn)),
            )
            ...
            if btn and btn.get("icon"):
                ...
                try:
                    await context.bot.copy_message(
                        chat_id=ch["id"], from_chat_id=draft["src_chat"], message_id=draft["src_msg"],
                        reply_markup=react_wrap_markup(_post_markup(btn, with_icon=False)),
                    )
...
# =========================================================
# 🔥 РЕАКЦИИ НА ПОСТЫ КАНАЛА — БОНУС К ШАНСУ
# =========================================================
#
# ВАЖНО про Telegram: реакции на посты каналов АНОНИМНЫ — бот (и даже админ канала)
# не узнаёт, кто именно поставил реакцию; то же верно для авто-копии поста в чате
# обсуждения. Поэтому «реакция» здесь — кнопка под постом (по нажатию Telegram
# присылает боту id человека), и она же обычно стоит рядом с обычными реакциями.
#
# Схема: бонус в процентных пунктах прибавляется СВЕРХУ к шансу (см. get_chance),
# действует react_hours часов, суммарно не больше react_max. За один пост человек
# получает бонус один раз (навсегда). Всё настраивается в админ-панели.
...
def _react_award(user_id: int, post_key: str) -> Tuple[str, float]:
    ...
    if got is None:
        got = react_awarded[post_key] = []
        while len(react_awarded) > REACT_MAX_POSTS_KEPT:
            react_awarded.pop(next(iter(react_awarded)))
    got.append(user_id)
    return "ok", add
...
async def _save_react_state() -> None:
    ...
    try:
        await db.set_setting(
            "react_state", json.dumps({"bonuses": bonuses, "awarded": react_awarded}, ensure_ascii=False)
        )
    except Exception:
...
def react_wrap_markup(markup: Optional[InlineKeyboardMarkup]) -> Optional[InlineKeyboardMarkup]:
    """Добавляет кнопку-реакцию отдельной строкой под уже существующие кнопки поста."""
    if not (S.get("react_enabled") and S.get("react_auto")):
        return markup
    rows = [list(r) for r in markup.inline_keyboard] if markup else []
    rows.append([_react_button()])
    return InlineKeyboardMarkup(rows)


async def react_channel_post_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Новый пост в привязанном канале → бот добавляет под него кнопку-реакцию,
    сохраняя кнопки, которые уже есть. Стоит в группе -3: иначе access_guard
    (группа -1) глотает все обновления из каналов, которых нет в списке чатов."""
    global _react_last_error
    msg = update.channel_post
    chat = update.effective_chat
    if not msg or not chat or not S.get("react_enabled") or not S.get("react_auto"):
        return
    if not any(c["id"] == chat.id for c in post_channels):
        return
    # служебные сообщения (закреп, смена названия…) и альбомы (у них кнопок не бывает)
    if not (msg.text or msg.caption or msg.effective_attachment) or getattr(msg, "media_group_id", None):
        return

    existing = [list(r) for r in msg.reply_markup.inline_keyboard] if msg.reply_markup else []
    if any(getattr(bt, "callback_data", None) == REACT_CB for row in existing for bt in row):
        return
    existing.append([_react_button()])
    try:
        await context.bot.edit_message_reply_markup(
            chat_id=chat.id, message_id=msg.message_id, reply_markup=InlineKeyboardMarkup(existing),
        )
    except BadRequest as e:
        if "not modified" not in str(e).lower():
            _react_last_error = f"{time.strftime('%d.%m %H:%M')} — {e}"
            log.warning("Реакции: не удалось добавить кнопку под пост %s/%s: %s", chat.id, msg.message_id, e)
    except TelegramError as e:
        _react_last_error = f"{time.strftime('%d.%m %H:%M')} — {e}"
        log.warning("Реакции: ошибка добавления кнопки: %s", e)
    except Exception:
        log.exception("Реакции: неожиданная ошибка в обработчике поста канала")


async def _react_linked_chat(bot, channel_id: int) -> Optional[int]:
    """Чат обсуждения, привязанный к каналу (кэш на час)."""
    hit = _react_linked_cache.get(channel_id)
    ...
    try:
        ...
        pass
    _react_linked_cache[channel_id] = (time.time(), linked)
    return linked


async def _react_targets(bot, channel_id: int) -> List[int]:
    """Куда писать сообщение: чат обсуждения этого канала, если он подключён к боту;
    иначе выбранный чат комментариев; иначе — подключённые чаты."""
    linked = await _react_linked_chat(bot, channel_id)
    if linked and linked in allowed_chat_ids:
        return [linked]
    cc = S.get("comment_chat_id")
    if cc and int(cc) in allowed_chat_ids:
        return [int(cc)]
...
async def _react_send(bot, chat_id: int, text: str, entities) -> None:
    photo = S.get("react_photo")
    try:
        if photo:
            await bot.send_photo(chat_id=chat_id, photo=photo, caption=text, caption_entities=entities or None)
        else:
            await bot.send_message(chat_id=chat_id, text=text, entities=entities or None)
    except TelegramError:
...
async def _react_announce(bot, user, added: float, channel_id: int, channel_title: str) -> None:
    ...
    for chat_id in await _react_targets(bot, channel_id):
        await _react_send(bot, chat_id, text, ents)
...
async def react_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    status, added = _react_award(user.id, f"{channel_id}:{msg.message_id}")
    ...
    await query.answer(
        f"🔥 +{fmt_num(added)}% к шансу на {fmt_num(S['react_hours'])} ч!\n🎯 Твой шанс: {chance:.2f}%"
    )
    _schedule_react_save()
    if S.get("react_announce"):
        _spawn(_react_announce(context.bot, user, added, channel_id, getattr(msg.chat, "title", "") or ""))


# ---------- меню в админ-панели ----------

def react_menu_keyboard() -> InlineKeyboardMarkup:
    on = "🟢 ВКЛ" if S["react_enabled"] else "🔴 ВЫКЛ"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(
        ...
        [InlineKeyboardButton(
            "🔘 Кнопка под новыми постами: " + ("🟢 ВКЛ" if S["react_auto"] else "🔴 ВЫКЛ"),
        ...
        [
            ...
            InlineKeyboardButton("♻️ Стандартный текст", callback_data="react_reset"),
        ],
        [InlineKeyboardButton("⬅️ Админ-панель", callback_data="main")],
    ])
...
async def show_react_menu(query, note: str = "") -> None:
    ...
    channels = (
        ", ".join(esc(c["title"]) for c in post_channels)
        if post_channels else "❌ не привязан (раздел «📢 Каналы и посты»)"
    )
    ...
    lines += [
        ...
        f"📢 Каналы: {channels}",
        "",
        f"Под постами привязанных каналов бот ставит кнопку «{esc(S['react_label'])}». "
        f"Человек нажимает её — получает {b('+' + fmt_num(S['react_bonus']) + '%')} к шансу "
        f"на {b(fmt_num(S['react_hours']) + ' ч')} (прибавляется сверху к обычному шансу, "
        f"суммарно не больше {b('+' + fmt_num(S['react_max']) + '%')}), а в чат приходит сообщение. "
        "За один пост — один бонус на человека.",
        "",
        "ℹ️ Почему кнопка: обычные реакции Telegram в каналах анонимны — бот не видит, кто "
        "их поставил. Нажатие кнопки однозначно определяет человека.",
        "",
        "Подстановки в тексте: <code>{user}</code> — упоминание, <code>{username}</code>, "
        "<code>{bonus}</code>, <code>{total}</code> — весь бонус сейчас, <code>{chance}</code> — "
        "шанс, <code>{hours}</code>, <code>{channel}</code>.",
        f"Сообщение: {'📷 + текст' if S.get('react_photo') else '📝 текст'}",
    ]
    if _react_last_error:
        lines += [
            "",
            f"⚠️ Последняя ошибка при добавлении кнопки: <code>{esc(_react_last_error[:200])}</code>\n"
            "Проверь, что бот — админ канала с правом «Редактирование сообщений».",
        ]
    await query.edit_message_text(
...
def main() -> None:
    ...
    # --- новые посты привязанных каналов: кнопка-реакция (раньше access_guard) ---
    app.add_handler(
        MessageHandler(filters.UpdateType.CHANNEL_POST, react_channel_post_handler), group=-3
    )
