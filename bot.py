...
def _build_leaderboard_message() -> Tuple[str, List[MessageEntity]]:
    ...
    if not top:
        ...
        for i, item in enumerate(top):
            ...
            name = item["name"] or (f"@{item['username']}" if item["username"] else str(item["user_id"]))
...
def top_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("♻️ Обнулить счётчики", callback_data="top_reset")],
        [InlineKeyboardButton("⬅️ Админ-панель", callback_data="main")],
    ])
...
async def show_top_menu(query) -> None:
    ...
    await query.edit_message_text(
        ...
        "(туда же можно вставить Premium Emoji), а имя игрока и счётчик "
        "бот подставляет сам.",
...
async def show_antispam_menu(query) -> None:
    ...
    await query.edit_message_text(
        ...
        f"{b(S['antispam_mute_minutes'])} мин с причиной «{ANTISPAM_REASON}».\n\n"
...
async def check_antispam(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        await message.reply_text(
            f"🔇 {user.mention_html()} получает мут на {mute_minutes} мин.\n"
            f"Причина: {ANTISPAM_REASON}.",
            parse_mode=ParseMode.HTML,
        )
    except TelegramError:
