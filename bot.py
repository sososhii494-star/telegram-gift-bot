...
_db_use_pg = False
_db_ready = False          # True только после успешной db_init_db()

_DB_JSON_SECTIONS = {
    "settings": dict, "stats": dict, "allowed_chats": list,
    "blocked_usernames": list, "boosts": dict, "payments": dict,
}


def _db_js(section: str):
    """Раздел JSON-хранилища. Создаёт его при отсутствии, поэтому KeyError
    (например, если бота остановили до конца инициализации) невозможен."""
    return _db_json_state.setdefault(section, _DB_JSON_SECTIONS[section]())
...
async def db_init_db() -> None:
    global _db_pool, _db_use_pg, _db_json_state, _db_ready

    if DATABASE_URL and "DATABASE_URL" not in os.environ:
        log.warning(
            "⚠️ Используется строка подключения к БД, зашитая в bot.py. Лучше задай "
            "переменную окружения DATABASE_URL на Render и смени пароль базы."
        )
    if DATABASE_URL:
        try:
            import asyncpg  # noqa
            ...
            # Render иногда «просыпается» дольше обычного — пробуем несколько раз,
            # прежде чем уйти на временный JSON (на Render его диск стирается).
            for attempt in range(1, 4):
                try:
                    ...
                    _db_pool = None
                    log.exception("PostgreSQL: попытка подключения %s/3 не удалась", attempt)
                    if attempt < 3:
            ...
            try:
                ...
                log.error(
                    "⚠️ РАБОТАЮ БЕЗ БАЗЫ: настройки и статистика пойдут во временный JSON и "
                    "пропадут при перезапуске. Проверь DATABASE_URL и статус базы на Render."
                )
                _db_pool = None
    ...
    if not isinstance(_db_json_state, dict):
        _db_json_state = {}
    for _name, _factory in _DB_JSON_SECTIONS.items():
        _db_json_state.setdefault(_name, _factory())
    _db_ready = True
...
async def _db_flush_json() -> None:
    if not _db_ready:
        # Состояние ещё не загружено из файла — запись затёрла бы сохранённые данные.
        return
...
async def db_set_setting(key: str, value: str) -> None:
    ...
    if _db_use_pg:
        ...
        if not _db_ready:
            log.warning("Хранилище не инициализировано — настройка %s не сохранена", key)
            return
        _db_js("settings")[key] = value
        await _db_flush_json()
...
async def db_get_setting(key: str, default=None):
    if _db_use_pg:
        ...
        if key not in _db_js("settings"):
            return default
        value = _db_js("settings")[key]
...
async def db_increment_stat(name: str, amount: int = 1) -> None:
    if _db_use_pg:
        ...
        _db_js("stats")[name] = int(_db_js("stats").get(name, 0)) + amount
        # JSON пишем не на каждое сообщение — иначе диск на Render не выдержит
        if int(_db_js("stats")[name]) % 20 == 0:
            await _db_flush_json()
...
async def db_load_stats() -> Dict[str, int]:
    ...
    return {k: int(v) for k, v in _db_js("stats").items()}
...
async def db_load_allowed_chats() -> List[int]:
    ...
    return [int(x) for x in _db_js("allowed_chats")]
...
async def db_add_allowed_chat(chat_id: int) -> None:
    if _db_use_pg:
        ...
        if int(chat_id) not in _db_js("allowed_chats"):
            _db_js("allowed_chats").append(int(chat_id))
            await _db_flush_json()
...
async def db_remove_allowed_chat(chat_id: int) -> None:
    if _db_use_pg:
        ...
        _db_json_state["allowed_chats"] = [
            x for x in _db_js("allowed_chats") if int(x) != int(chat_id)
        ]
        await _db_flush_json()
...
async def db_load_active_boosts() -> Dict[int, Dict[str, float]]:
    ...
    alive = {
        int(uid): data
        for uid, data in _db_js("boosts").items()
        if float(data.get("expires_at", 0)) > now
    }
    if len(alive) != len(_db_js("boosts")):
...
async def db_drop_boost(user_id: int) -> None:
    if _db_use_pg:
        ...
        _db_js("boosts").pop(str(user_id), None)
        await _db_flush_json()
...
async def db_get_payment(charge_id: str) -> Optional[Dict]:
    ...
    return _db_js("payments").get(charge_id)
...
async def db_last_payments(limit: int = 10) -> List[Dict]:
    ...
    items = [
        dict(charge_id=k, **v) for k, v in _db_js("payments").items()
    ]
...
# Профили пользователей: 10 уровней, 2 уровень открывается с 100 XP.
...
# Строка «когда вступил в чат» — отдельный пункт админки (жирный шрифт, Premium Emoji).
DEFAULT_PROFILE_JOINED_TEXT = "📅 В чате с: {joined}"
# Часовой пояс для даты в профиле (часы от UTC). 3 = Москва.
PROFILE_TZ_HOURS = 3
PROFILE_STATUS_MAX_LEN = 120
_RU_MONTHS_GEN = [
...
# =========================================================
# ЧАТ-МОДЕРАЦИЯ: РЕДАКТИРУЕМЫЕ ТЕКСТЫ ПО УМОЛЧАНИЮ
# (жирный шрифт и Premium Emoji задаются в админ-панели так же, как у
# остальных сообщений бота — этот текст лишь запасной, пока не изменён)
# =========================================================
DEFAULT_MOD_MUTE_TEXT = "🔇 {user} получает мут.\nСрок: {duration}\nПричина: {reason}"
DEFAULT_MOD_UNMUTE_TEXT = "🔊 С {user} снят мут."
DEFAULT_MOD_BAN_TEXT = "🚫 {user} отправляется гулять.\nСрок: {duration}\nПричина: {reason}"
DEFAULT_MOD_UNBAN_TEXT = "✅ {user} разбанен и может вернуться в чат."
DEFAULT_MOD_ADMIN_ADD_TEXT = "🛡 {user} назначен админом этого чата."
DEFAULT_MOD_ADMIN_DEL_TEXT = "🚷 {user} больше не админ этого чата."
DEFAULT_MOD_ADMINS_HEADER_TEXT = "🛡 Админы этого чата"
DEFAULT_MOD_PUNISHED_HEADER_TEXT = "🚫 Наказанные в этом чате"
...
S: Dict[str, object] = {
    ...
    "profile_joined_text": DEFAULT_PROFILE_JOINED_TEXT,
    "profile_joined_entities": [],
    ...
    # Чат-модерация: мут/бан/размут/разбан/админы чата/наказанные — все ответы
    # бота редактируются в админке (жирный шрифт, Premium Emoji, {user}/{duration}/{reason}).
    "mod_mute_text": DEFAULT_MOD_MUTE_TEXT,
    "mod_mute_photo": None,
    "mod_mute_entities": [],
    "mod_unmute_text": DEFAULT_MOD_UNMUTE_TEXT,
    "mod_unmute_photo": None,
    "mod_unmute_entities": [],
    "mod_ban_text": DEFAULT_MOD_BAN_TEXT,
    "mod_ban_photo": None,
    "mod_ban_entities": [],
    "mod_unban_text": DEFAULT_MOD_UNBAN_TEXT,
    "mod_unban_photo": None,
    "mod_unban_entities": [],
    "mod_admin_add_text": DEFAULT_MOD_ADMIN_ADD_TEXT,
    "mod_admin_add_photo": None,
    "mod_admin_add_entities": [],
    "mod_admin_del_text": DEFAULT_MOD_ADMIN_DEL_TEXT,
    "mod_admin_del_photo": None,
    "mod_admin_del_entities": [],
    "mod_admins_header_text": DEFAULT_MOD_ADMINS_HEADER_TEXT,
    "mod_admins_header_photo": None,
    "mod_admins_header_entities": [],
    "mod_punished_header_text": DEFAULT_MOD_PUNISHED_HEADER_TEXT,
    "mod_punished_header_photo": None,
    "mod_punished_header_entities": [],
}
...
# АДМИНЫ ЧАТА (назначаются владельцем командой «админ», отдельно по каждому чату):
# chat_id -> {user_id: отображаемое имя}. В отличие от bot_admin_ids/бот-админов,
# у них нет доступа к админ-панели — только к командам модерации в самом чате.
chat_admin_ids: Dict[int, Dict[int, str]] = {}

# НАКАЗАННЫЕ (мут/бан) по каждому чату: chat_id -> {user_id: {...}}.
# type: "mute"/"ban", until: unix-время окончания (0 — навсегда), at: когда выдано,
# by: кто выдал, reason: причина, name: отображаемое имя на момент наказания.
punished: Dict[int, Dict[int, dict]] = {}
...
# Ожидание фото для персонального фона профиля: user_id -> {"chat_id", "until"}.
profile_photo_wait: Dict[int, Dict[str, Any]] = {}
# Ожидание нового статуса от пользователя: user_id -> {"chat_id", "until"}.
profile_status_wait: Dict[int, Dict[str, Any]] = {}
PROFILE_PHOTO_WAIT_SECONDS = 120
...
def _spawn(coro) -> None:
    ...
    def _done(t) -> None:
        ...
        if not t.cancelled() and t.exception() is not None:
            log.error("Фоновая задача завершилась с ошибкой", exc_info=t.exception())
...
def render_template(text: str, entities, values: Dict[str, str], mention_user=None, value_entities=None):
    ...
    # Entities внутри самих значений (например Premium Emoji в статусе пользователя):
    # offset'ы в value_entities[ключ] считаются от начала значения.
    if value_entities:
        delta = 0
        for (start, len_old, new, _is_user), key in zip(repls, repl_keys):
            new_len = _u16(new)
            for e in value_entities.get(key) or []:
                if e.offset + e.length <= new_len:
                    new_entities.append(_clone_entity_len(e, start + delta + e.offset, e.length))
            delta += new_len - len_old
...
async def load_persistent_state() -> None:
    ...
    S["profile_joined_text"] = await db.get_setting("profile_joined_text", DEFAULT_PROFILE_JOINED_TEXT) or DEFAULT_PROFILE_JOINED_TEXT
    S["profile_joined_entities"] = _entities_from_json(await db.get_setting("profile_joined_entities", "[]"))
            ...
            for uid, rec in data_profiles.items():
                try:
                    profile_users[int(uid)] = {
                        ...
                        "photo": str(rec.get("photo") or "") or None,
                        "joined_at": float(rec.get("joined_at") or 0) or None,
                        "status": str(rec.get("status") or "")[:PROFILE_STATUS_MAX_LEN] or None,
                        "status_entities": str(rec.get("status_entities") or ""),
                    }
                except (TypeError, ValueError, AttributeError):
                    continue
    except Exception:
    ...
    chat_admin_ids.clear()
    try:
        raw_ca = await db.get_setting("chat_admins", "{}")
        data_ca = json.loads(raw_ca) if isinstance(raw_ca, str) else raw_ca
        if isinstance(data_ca, dict):
            for cid, admins in data_ca.items():
                if isinstance(admins, dict):
                    chat_admin_ids[int(cid)] = {
                        int(uid): str(name or "") for uid, name in admins.items()
                    }
    except Exception:
        log.exception("Не удалось загрузить админов чатов")

    punished.clear()
    try:
        raw_pu = await db.get_setting("chat_punished", "{}")
        data_pu = json.loads(raw_pu) if isinstance(raw_pu, str) else raw_pu
        if isinstance(data_pu, dict):
            for cid, users in data_pu.items():
                if isinstance(users, dict):
                    punished[int(cid)] = {
                        int(uid): rec for uid, rec in users.items() if isinstance(rec, dict)
                    }
    except Exception:
        log.exception("Не удалось загрузить список наказанных")

    for _pref, _default in (
        ("mod_mute", DEFAULT_MOD_MUTE_TEXT),
        ("mod_unmute", DEFAULT_MOD_UNMUTE_TEXT),
        ("mod_ban", DEFAULT_MOD_BAN_TEXT),
        ("mod_unban", DEFAULT_MOD_UNBAN_TEXT),
        ("mod_admin_add", DEFAULT_MOD_ADMIN_ADD_TEXT),
        ("mod_admin_del", DEFAULT_MOD_ADMIN_DEL_TEXT),
        ("mod_admins_header", DEFAULT_MOD_ADMINS_HEADER_TEXT),
        ("mod_punished_header", DEFAULT_MOD_PUNISHED_HEADER_TEXT),
    ):
        S[f"{_pref}_text"] = await db.get_setting(f"{_pref}_text", _default)
        S[f"{_pref}_photo"] = await db.get_setting(f"{_pref}_photo", None)
        S[f"{_pref}_entities"] = _entities_from_json(
            await db.get_setting(f"{_pref}_entities", "[]")
        )
...
async def _save_chat_admins() -> None:
    await _db_set("chat_admins", json.dumps(
        {str(cid): {str(uid): name for uid, name in admins.items()}
         for cid, admins in chat_admin_ids.items()},
        ensure_ascii=False,
    ))


async def _save_punished() -> None:
    await _db_set("chat_punished", json.dumps(
        {str(cid): {str(uid): rec for uid, rec in users.items()}
         for cid, users in punished.items()},
        ensure_ascii=False,
    ))
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if payload.startswith("vip:"):
        ...
        if expires == 0:
            ...
            expires_text = time.strftime("%d.%m.%Y %H:%M", time.localtime(expires))
...
def admin_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton(
                f"🛡 Модерация чата ({sum(len(v) for v in chat_admin_ids.values())} админов)",
                callback_data="chatmod_menu",
            )],
            [InlineKeyboardButton(
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "profile_message":
        ...
        await query.edit_message_text(
            ...
            "<code>{next_xp}</code> — XP до следующего уровня\n"
    ...
    if data == "profile_joined":
        _clear_waiting(context)
        context.user_data["waiting_profile_joined"] = True
        await query.edit_message_text(
            f"📅 {b('СТРОКА «В ЧАТЕ С»')}\n\n"
            "Эта строка добавляется в конец профиля. Отправь текст — жирный шрифт, "
            "курсив и Premium Emoji сохранятся.\n\n"
            "Обязательная подстановка: <code>{joined}</code> — дата и время вступления, "
            "например «24 сентября, 14:35».\n\n"
            f"Сейчас: {esc(S.get('profile_joined_text') or DEFAULT_PROFILE_JOINED_TEXT)}\n\n"
            "❌ /cancel — отменить.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("♻️ Стандартная строка", callback_data="profile_joined_reset")],
                [InlineKeyboardButton("⬅️ Назад", callback_data="profile_menu")],
            ]),
            parse_mode=ParseMode.HTML,
        )
    ...
    if data == "profile_joined_reset":
        _clear_waiting(context)
        S["profile_joined_text"] = DEFAULT_PROFILE_JOINED_TEXT
        S["profile_joined_entities"] = []
        await _db_set("profile_joined_text", S["profile_joined_text"])
        await _db_set_entities("profile_joined_entities", [])
        await show_profile_menu(query, note="✅ Строка «в чате с» сброшена к стандартной.")
        return
    ...
    # ---------------- МОДЕРАЦИЯ ЧАТА ----------------
    if data == "chatmod_menu":
        _clear_waiting(context)
        await show_chatmod_menu(query)
        return

    if data in (
        "mod_mute_msg", "mod_unmute_msg", "mod_ban_msg", "mod_unban_msg",
        "mod_admin_add_msg", "mod_admin_del_msg",
        "mod_admins_header_msg", "mod_punished_header_msg",
    ):
        _clear_waiting(context)
        context.user_data["waiting_" + data] = True
        placeholders = "<code>{user}</code>"
        if data in ("mod_mute_msg", "mod_ban_msg"):
            placeholders += ", <code>{duration}</code>, <code>{reason}</code>"
        await query.edit_message_text(
            f"📝 {b('ТЕКСТ')}\n\n"
            "Отправь текст или фото с подписью. Жирный шрифт и Premium Emoji сохранятся.\n"
            f"Плейсхолдеры: {placeholders}.\n\n❌ /cancel — отменить.",
            reply_markup=back_kb("chatmod_menu"), parse_mode=ParseMode.HTML,
        )
        return

    if data == "chatmod_reset":
        _clear_waiting(context)
        for pref, default in (
            ("mod_mute", DEFAULT_MOD_MUTE_TEXT),
            ("mod_unmute", DEFAULT_MOD_UNMUTE_TEXT),
...
def profile_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✏️ Изменить текст / фото", callback_data="profile_message")],
        ...
        [InlineKeyboardButton("📅 Строка «в чате с» (дата)", callback_data="profile_joined")],
...
async def show_profile_menu(query, note: str = "") -> None:
    ...
    text = (
        ...
        f"⏱ XP: каждый час активному пользователю случайно +{PROFILE_XP_MIN}–{PROFILE_XP_MAX}\n"
...
def _profile_record(user) -> Dict[str, Any]:
    ...
    if rec is None:
        rec = {"xp": 0, "last_xp_hour": -1, "last_activity": now, "created_at": now}
...
def _profile_vip_expires(user) -> str:
    ...
    if expires <= time.time():
        return "—"
    return time.strftime("%d.%m.%Y %H:%M", time.localtime(expires))


def _profile_values(user) -> Dict[str, str]:
...
async def _save_profiles() -> None:
    try:
        await db.set_setting("profile_users", json.dumps({str(k): v for k, v in profile_users.items()}, ensure_ascii=False))
...
async def profile_xp_loop() -> None:
    """Раз в час активным пользователям выдаёт случайные 3–7 XP."""
    while True:
        try:
            await asyncio.sleep(PROFILE_XP_INTERVAL_SECONDS)
        except asyncio.CancelledError:
            raise
        now = time.time()
        current_hour = int(now // 3600)
        changed = False
        for rec in profile_users.values():
            if now - float(rec.get("last_activity", 0)) > 2 * 3600:
                continue
            if int(rec.get("last_xp_hour", -1)) >= current_hour:
                continue
            rec["xp"] = max(0, int(rec.get("xp", 0))) + random.randint(PROFILE_XP_MIN, PROFILE_XP_MAX)
            rec["last_xp_hour"] = current_hour
            changed = True
        if changed:
            await _save_profiles()


def _pset_main_kb(uid: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⚙️ Настройки профиля", callback_data=f"pset:menu:{uid}")],
    ])


def _pset_menu_kb(uid: int, has_photo: bool, has_status: bool = False) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton("🖼 Загрузить своё фото", callback_data=f"pset:photo:{uid}")],
        [InlineKeyboardButton("📝 Изменить статус", callback_data=f"pset:status:{uid}")],
    ]
    if has_photo:
        rows.append([InlineKeyboardButton("🗑 Убрать моё фото", callback_data=f"pset:reset:{uid}")])
    if has_status:
        rows.append([InlineKeyboardButton("🗑 Сбросить мой статус", callback_data=f"pset:sreset:{uid}")])
    rows.append([InlineKeyboardButton("⬅️ Назад", callback_data=f"pset:back:{uid}")])
    return InlineKeyboardMarkup(rows)


def _fmt_join_date(ts) -> str:
    """«24 сентября, 14:35» (год добавляется, если он не текущий)."""
    from datetime import datetime, timedelta, timezone
    tz = timezone(timedelta(hours=PROFILE_TZ_HOURS))
    try:
        dt = datetime.fromtimestamp(float(ts), tz)
    except (TypeError, ValueError, OverflowError, OSError):
        return "—"
    now = datetime.now(tz)
    day = f"{dt.day} {_RU_MONTHS_GEN[dt.month - 1]}"
    if dt.year != now.year:
        day += f" {dt.year}"
    return f"{day}, {dt:%H:%M}"


async def profile_join_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Запоминает время вступления участников в чат (для строки «в чате с»)."""
    message = update.message
    ...
    for member in message.new_chat_members:
        if member.is_bot:
            continue
        rec = _profile_record(member)
        rec["joined_at"] = ts
    _schedule_profile_save()


async def show_user_profile(message, user) -> None:
    rec = _profile_record(user)
    rec["last_activity"] = time.time()
    _schedule_profile_save()

    # Premium Emoji в личном статусе — только для VIP (и только пока VIP активен).
    status_ents: List[MessageEntity] = []
    if rec.get("status") and rec.get("status_entities") and _profile_is_vip(user):
        status_ents = _entities_from_json(rec["status_entities"])

    text, entities = render_template(
        str(S.get("profile_text") or DEFAULT_PROFILE_TEXT),
        S.get("profile_entities") or [],
        _profile_values(user),
        value_entities={"status": status_ents} if status_ents else None,
    )

    # Строка «в чате с» — настраивается в админке отдельным пунктом.
    joined_ts = rec.get("joined_at") or rec.get("created_at") or time.time()
    j_text, j_ents = render_template(
        str(S.get("profile_joined_text") or DEFAULT_PROFILE_JOINED_TEXT),
        S.get("profile_joined_entities") or [],
        {"joined": _fmt_join_date(joined_ts)},
    )
    if j_text:
        text, entities = _concat_parts([(text, entities), ("\n", []), (j_text, j_ents)])

    kb = _pset_main_kb(user.id)
    # У подписи к фото лимит 1024 символа — подрезаем, чтобы профиль не пропадал.
    cap_text, cap_ents = _truncate_u16(text, entities, 1024)
    try:
        # Персональное фото пользователя имеет приоритет над общим фото профиля.
        custom = rec.get("photo")
        if custom:
            try:
                await message.reply_photo(
                    photo=custom, caption=cap_text, caption_entities=cap_ents or None, reply_markup=kb
                )
                return
            except BadRequest as e:
                err = str(e).lower()
                if "file" in err or "identifier" in err:
                    # file_id стал недействителен — убираем и показываем стандартный профиль.
                    log.warning("Персональное фото профиля %s недействительно, сбрасываю", user.id)
                    rec["photo"] = None
                    _schedule_profile_save()
                else:
                    raise
        if S.get("profile_photo"):
            await message.reply_photo(
                photo=S["profile_photo"], caption=cap_text, caption_entities=cap_ents or None, reply_markup=kb
            )
        else:
            full_text, full_ents = _truncate_u16(text, entities, 4096)
            await message.reply_text(full_text, entities=full_ents or None, reply_markup=kb)
    except TelegramError:
        log.exception("Не удалось отправить профиль пользователя")


async def _profile_photo_receive(message, user, chat) -> bool:
    """Если пользователь ждёт загрузки фона профиля — сохраняет присланное фото."""
    wait = profile_photo_wait.get(user.id)
    if not wait:
        return False
    if time.time() > float(wait.get("until", 0)):
        profile_photo_wait.pop(user.id, None)
        return False
    if int(wait.get("chat_id", 0)) != chat.id:
        return False
    profile_photo_wait.pop(user.id, None)
    rec = _profile_record(user)
    rec["photo"] = message.photo[-1].file_id
    await _save_profiles()
    try:
        await message.reply_text("✅ Готово! Твоё фото установлено — вот как теперь выглядит профиль:")
    except TelegramError:
        pass
...
async def _profile_status_receive(message, user, chat) -> bool:
    """Если пользователь ждёт нового статуса — сохраняет присланный текст.
    Обычным пользователям — только текст; Premium Emoji сохраняются только у VIP."""
    wait = profile_status_wait.get(user.id)
    if not wait:
        return False
    if time.time() > float(wait.get("until", 0)):
        profile_status_wait.pop(user.id, None)
        return False
    if int(wait.get("chat_id", 0)) != chat.id:
        return False
    ...
    raw = message.text or ""
    value = raw.strip()
    if not value or value in ("Я", "я"):
        return False  # «Я» по-прежнему открывает профиль, а не становится статусом
    if "\n" in value:
        await message.reply_text("❌ Статус — одной строкой, без переносов. Отправь ещё раз.")
        return True
    if len(value) > PROFILE_STATUS_MAX_LEN:
        await message.reply_text(
            f"❌ Слишком длинно: {len(value)} из {PROFILE_STATUS_MAX_LEN} символов. Сократи и отправь ещё раз."
        )
        return True

    is_vip = _profile_is_vip(user)
    kept: List[MessageEntity] = []
    has_custom = False
    lead = _u16(raw[: len(raw) - len(raw.lstrip())])
    width = _u16(value)
    for e in message.entities or []:
        if e.type != MessageEntity.CUSTOM_EMOJI:
            continue
        has_custom = True
        if is_vip and e.offset >= lead and e.offset + e.length <= lead + width:
            kept.append(_clone_entity_len(e, e.offset - lead, e.length))

    profile_status_wait.pop(user.id, None)
    rec = _profile_record(user)
    rec["status"] = value
    rec["status_entities"] = _entities_to_json(kept) if kept else ""
    # Личный статус заменяет назначенный админом.
    profile_statuses.pop(user.id, None)
    await _save_profiles()
    await _save_vip_state()

    note = ""
    if has_custom and not is_vip:
        note = "\n💎 Premium-эмодзи в статусе доступны только VIP — они показаны как обычные символы."
    try:
        await message.reply_text(f"✅ Статус обновлён.{note}")
    except TelegramError:
        pass
    await show_user_profile(message, user)
    return True
...
async def profile_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query:
        return
    ...
    if user.id != owner_id:
        await query.answer("Это не твой профиль 🙅 Напиши «Я», чтобы открыть свой.", show_alert=True)
        return

    rec = _profile_record(user)

    async def _set_kb(kb: InlineKeyboardMarkup) -> None:
        try:
    ...
    if action == "menu":
        await query.answer()
        await _set_kb(_pset_menu_kb(user.id, bool(rec.get("photo")), bool(rec.get("status"))))
    elif action == "back":
        await query.answer()
        await _set_kb(_pset_main_kb(user.id))
    elif action == "photo":
        msg = query.message
        if not msg:
            await query.answer()
            return
        profile_photo_wait[user.id] = {
            "chat_id": msg.chat_id,
            "until": time.time() + PROFILE_PHOTO_WAIT_SECONDS,
        }
        profile_status_wait.pop(user.id, None)
        await query.answer()
        await _set_kb(_pset_main_kb(user.id))
        try:
            await msg.reply_text(
                f"🖼 {user.mention_html()}, отправь в этот чат фото — оно станет фоном твоего профиля.\n"
                f"Жду {PROFILE_PHOTO_WAIT_SECONDS // 60} мин. Отмена — просто ничего не присылай.",
                parse_mode=ParseMode.HTML,
            )
        except TelegramError:
            log.exception("Не удалось отправить приглашение загрузить фото профиля")
    elif action == "status":
        msg = query.message
        if not msg:
            await query.answer()
            return
        profile_status_wait[user.id] = {
            "chat_id": msg.chat_id,
            "until": time.time() + PROFILE_PHOTO_WAIT_SECONDS,
        }
        profile_photo_wait.pop(user.id, None)
        ...
        await _set_kb(_pset_main_kb(user.id))
        vip_line = (
            "💎 Ты VIP — можно добавлять Premium-эмодзи."
            if _profile_is_vip(user)
            else "Только текст. Premium-эмодзи в статусе доступны VIP 💎"
        )
        try:
            await msg.reply_text(
                f"📝 {user.mention_html()}, отправь в этот чат новый статус — одной строкой, "
                f"до {PROFILE_STATUS_MAX_LEN} символов.\n{vip_line}\n"
                f"Жду {PROFILE_PHOTO_WAIT_SECONDS // 60} мин. Отмена — просто ничего не присылай.",
                parse_mode=ParseMode.HTML,
            )
        except TelegramError:
            log.exception("Не удалось отправить приглашение сменить статус")
    elif action == "reset":
        rec["photo"] = None
        await _save_profiles()
        await query.answer("Фото убрано, вернулся стандартный фон", show_alert=False)
        await _set_kb(_pset_main_kb(user.id))
    elif action == "sreset":
        rec["status"] = None
        rec["status_entities"] = ""
        await _save_profiles()
        await query.answer("Статус сброшен", show_alert=False)
        await _set_kb(_pset_main_kb(user.id))
    else:
        await query.answer()
...
# =========================================================
# ЧАТ-МОДЕРАЦИЯ: ПОМОЛЧИ / ГУЛЯЙ / РАЗМУТ / РАЗБАН / АДМИНЫ ЧАТА / НАКАЗАННЫЕ
# =========================================================
#
# Команды (без слэша, прямо текстом в чате):
#   помолчи   — мут   (ответом на сообщение, либо «помолчи @user 10m причина»)
#   размут    — снять мут
#   гуляй     — бан    (те же варианты; срок необязателен)
#   разбан    — снять бан
#   админ     — назначить админа ЭТОГО чата (только владелец бота)
#   разадмин  — снять админа этого чата (только владелец бота)
#   админы / список админов — список админов чата
#   наказанные — список замьюченных и забаненных в этом чате
#
# Мут/бан/размут/разбан и оба списка доступны владельцу бота, добавленным
# админам бота (/botadmin) и назначенным админам конкретного чата (команда
# «админ»). Назначать и снимать админов чата может только владелец бота —
# у самих админов чата нет доступа к админ-панели и к управлению другими
# админами, только к перечисленным выше командам прямо в чате.

_DUR_RE = re.compile(r"^(\d+)\s*([mhdмчд])\.?$", re.IGNORECASE)
_DUR_MULT = {"m": 60, "м": 60, "h": 3600, "ч": 3600, "d": 86400, "д": 86400}
_DUR_WORDS_PERM = ("навсегда", "нав", "перм", "permanent", "forever")


def is_chat_mod(update: Update) -> bool:
    """Владелец бота, любой добавленный админ бота или назначенный админ ИМЕННО этого чата."""
    user = update.effective_user
    chat = update.effective_chat
    if not user or not chat:
        return False
    if is_admin(update) or is_bot_admin_user(user):
        return True
    return user.id in chat_admin_ids.get(chat.id, {})
...
def _parse_duration(token: str) -> Optional[int]:
    """None — токен не похож на срок (значит это часть причины).
    0 — «навсегда». >0 — срок в секундах."""
    if not token:
        return None
    low = token.strip().lower()
    if low in _DUR_WORDS_PERM:
        return 0
    m = _DUR_RE.match(low)
    if not m:
        return None
    return int(m.group(1)) * _DUR_MULT.get(m.group(2).lower(), 60)


async def _resolve_mod_target(message, context, remainder: str):
    """Определяет цель модерации: по ответу на сообщение или по первому слову
    после команды (@username / ID). Возвращает (user_id, имя_для_показа,
    telegram.User-объект_для_упоминания, остаток_текста)."""
    reply = message.reply_to_message
    if reply and reply.from_user:
        u = reply.from_user
        return u.id, (u.full_name or (f"@{u.username}" if u.username else str(u.id))), u, remainder.strip()

    tokens = remainder.split(maxsplit=1)
    if not tokens:
        raise RuntimeError(
            "Укажи, кого — ответь этой командой на сообщение человека, либо "
            "первым словом добавь @username или ID."
        )
    first, tail = tokens[0], (tokens[1] if len(tokens) > 1 else "")
    if not (first.startswith("@") or first.lstrip("-").isdigit()):
        raise RuntimeError(
            "Укажи, кого — ответь этой командой на сообщение человека, либо "
            "первым словом добавь @username или ID."
        )
    uid, display = await _resolve_chat_user(context.bot, first)
    d = known_users.get(uid, {})
    mention = User(
        id=uid, is_bot=False,
        first_name=d.get("n") or display.lstrip("@") or str(uid),
        username=d.get("u") or None,
    )
    return uid, display, mention, tail.strip()


async def _reply_mod_text(message, prefix: str, default_text: str, values: Dict[str, str],
                          mention_user=None) -> None:
    """Отправляет редактируемый ответ модерации (текст/фото/жирный/Premium Emoji)."""
    text = str(S.get(f"{prefix}_text") or default_text)
    ents = S.get(f"{prefix}_entities")
    text, ents = render_template(text, ents, values, mention_user=mention_user)
    photo = S.get(f"{prefix}_photo")
    try:
        if photo:
            await message.reply_photo(photo=photo, caption=text, caption_entities=ents or None)
        else:
            await message.reply_text(text, entities=ents or None)
    except TelegramError:
        log.exception("Не удалось отправить сообщение модерации «%s»", prefix)


def _set_punishment(chat_id: int, uid: int, kind: str, until: int, by: int, reason: str, name: str) -> None:
    rec = punished.setdefault(chat_id, {})
    rec[uid] = {
        "type": kind, "until": int(until), "at": int(time.time()),
        "by": by, "reason": reason, "name": name,
    }
    _spawn(_save_punished())


def _clear_punishment(chat_id: int, uid: int, kind: Optional[str] = None) -> None:
    rec = punished.get(chat_id)
    if not rec or uid not in rec:
        return
    if kind and rec[uid].get("type") != kind:
        return
    del rec[uid]
    _spawn(_save_punished())


async def _handle_mod_mute(message, context, chat_id: int, remainder: str) -> None:
    try:
        uid, display, mention, tail = await _resolve_mod_target(message, context, remainder)
    except RuntimeError as e:
    ...
    try:
        await context.bot.restrict_chat_member(
            chat_id=chat_id, user_id=uid, permissions=CLOSED_PERMS,
            until_date=until or None,
        )
    except TelegramError as e:
        await message.reply_text(f"❌ Не удалось замьютить.\n<code>{esc(e)}</code>", parse_mode=ParseMode.HTML)
        return

    _set_punishment(chat_id, uid, "mute", until, message.from_user.id, reason, display)
    await _reply_mod_text(
        message, "mod_mute", DEFAULT_MOD_MUTE_TEXT,
        {"user": display, "duration": _fmt_duration(duration), "reason": reason}, mention,
    )


async def _handle_mod_unmute(message, context, chat_id: int, remainder: str) -> None:
    try:
        uid, display, mention, _tail = await _resolve_mod_target(message, context, remainder)
    except RuntimeError as e:
        await message.reply_text(f"❌ {e}")
        return
    try:
        await context.bot.restrict_chat_member(chat_id=chat_id, user_id=uid, permissions=_full_perms())
    except TelegramError as e:
        await message.reply_text(f"❌ Не удалось снять мут.\n<code>{esc(e)}</code>", parse_mode=ParseMode.HTML)
        return
    _clear_punishment(chat_id, uid, "mute")
    await _reply_mod_text(message, "mod_unmute", DEFAULT_MOD_UNMUTE_TEXT, {"user": display}, mention)


async def _handle_mod_ban(message, context, chat_id: int, remainder: str) -> None:
    try:
        uid, display, mention, tail = await _resolve_mod_target(message, context, remainder)
    except RuntimeError as e:
        await message.reply_text(f"❌ {e}")
    ...
    until = int(time.time()) + duration if duration > 0 else 0
    try:
        await context.bot.ban_chat_member(chat_id=chat_id, user_id=uid, until_date=until or None)
    except TelegramError as e:
        await message.reply_text(f"❌ Не удалось забанить.\n<code>{esc(e)}</code>", parse_mode=ParseMode.HTML)
        return

    _set_punishment(chat_id, uid, "ban", until, message.from_user.id, reason, display)
    await _reply_mod_text(
        message, "mod_ban", DEFAULT_MOD_BAN_TEXT,
        {"user": display, "duration": _fmt_duration(duration), "reason": reason}, mention,
    )


async def _handle_mod_unban(message, context, chat_id: int, remainder: str) -> None:
    try:
        uid, display, mention, _tail = await _resolve_mod_target(message, context, remainder)
    except RuntimeError as e:
        await message.reply_text(f"❌ {e}")
    ...
    try:
        ...
        await message.reply_text(f"❌ Не удалось разбанить.\n<code>{esc(e)}</code>", parse_mode=ParseMode.HTML)
        return
    _clear_punishment(chat_id, uid, "ban")
    await _reply_mod_text(message, "mod_unban", DEFAULT_MOD_UNBAN_TEXT, {"user": display}, mention)


async def _handle_mod_admin_add(message, context, chat_id: int, remainder: str) -> None:
    try:
        uid, display, mention, _tail = await _resolve_mod_target(message, context, remainder)
    except RuntimeError as e:
        await message.reply_text(f"❌ {e}")
        return
    admins = chat_admin_ids.setdefault(chat_id, {})
    admins[uid] = display
    _spawn(_save_chat_admins())
    await _reply_mod_text(message, "mod_admin_add", DEFAULT_MOD_ADMIN_ADD_TEXT, {"user": display}, mention)


async def _handle_mod_admin_del(message, context, chat_id: int, remainder: str) -> None:
    try:
        uid, display, mention, _tail = await _resolve_mod_target(message, context, remainder)
    except RuntimeError as e:
        await message.reply_text(f"❌ {e}")
        return
    admins = chat_admin_ids.get(chat_id)
    if not admins or uid not in admins:
        await message.reply_text("ℹ️ Этот человек и так не админ этого чата.")
        return
    del admins[uid]
    _spawn(_save_chat_admins())
    await _reply_mod_text(message, "mod_admin_del", DEFAULT_MOD_ADMIN_DEL_TEXT, {"user": display}, mention)


async def _handle_mod_admins_list(message, chat_id: int) -> None:
    admins = chat_admin_ids.get(chat_id) or {}
    header = str(S.get("mod_admins_header_text") or DEFAULT_MOD_ADMINS_HEADER_TEXT)
    header_ents = S.get("mod_admins_header_entities") or []
    if not admins:
        body = "\n\nПока никого. Назначить: ответь на сообщение человека словом «админ»."
    else:
        lines = [f"• {name or str(uid)} — ID {uid}" for uid, name in admins.items()]
        body = "\n\n" + "\n".join(lines)
    text, ents = append_bold(header, header_ents, body)
    try:
        await message.reply_text(text, entities=ents or None)
    except TelegramError:
        log.exception("Не удалось отправить список админов чата")


async def _handle_mod_punished_list(message, chat_id: int) -> None:
    records = punished.get(chat_id) or {}
    now = time.time()
    # Истёкшие мут/бан Telegram снимает сам — чистим список от таких записей,
    # чтобы «наказанные» не показывали тех, кто уже свободен.
    expired = [uid for uid, rec in records.items()
               if rec.get("until") and rec["until"] <= now]
    if expired:
        for uid in expired:
            records.pop(uid, None)
        _spawn(_save_punished())

    header = str(S.get("mod_punished_header_text") or DEFAULT_MOD_PUNISHED_HEADER_TEXT)
    header_ents = S.get("mod_punished_header_entities") or []
    if not records:
        body = "\n\nПока никого."
    else:
        lines = []
        for uid, rec in records.items():
            kind = "🔇 мут" if rec.get("type") == "mute" else "🚫 бан"
            until = rec.get("until") or 0
            left = "навсегда" if not until else _fmt_duration(max(0, int(until - now)))
            name = rec.get("name") or str(uid)
            reason = rec.get("reason") or "не указана"
            lines.append(f"• {kind} — {name} (ID {uid}) — {left}\n  Причина: {reason}")
        body = "\n\n" + "\n\n".join(lines)
    text, ents = append_bold(header, header_ents, body)
    try:
        await message.reply_text(text, entities=ents or None)
    except TelegramError:
        log.exception("Не удалось отправить список наказанных")
...
_MOD_COMMANDS = {
    ...
    "разадмин": "admin_del",
    "админы": "admins_list",
    "наказанные": "punished_list",
}
_MOD_OWNER_ONLY_ACTIONS = ("admin_add", "admin_del")
_MOD_FIRST_WORD_RE = re.compile(r"^(\S+)")


async def chat_mod_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    message = update.message
    chat = update.effective_chat
    user = update.effective_user
    ...
    if low.startswith("список админов"):
        action = "admins_list"
        rest = text_raw[len("список админов"):]
    else:
        m = _MOD_FIRST_WORD_RE.match(text_raw)
        if not m:
            return
        first_word = m.group(1)
        cmd = first_word.lower().strip(".,!?;:")
        action = _MOD_COMMANDS.get(cmd)
        if not action:
            return
        rest = text_raw[len(first_word):]

    if action in _MOD_OWNER_ONLY_ACTIONS:
        if not is_admin(update):
            return
    else:
        if not is_chat_mod(update):
            return

    try:
        if action == "mute":
            await _handle_mod_mute(message, context, chat.id, rest)
        elif action == "unmute":
            await _handle_mod_unmute(message, context, chat.id, rest)
        elif action == "ban":
            await _handle_mod_ban(message, context, chat.id, rest)
        elif action == "unban":
            await _handle_mod_unban(message, context, chat.id, rest)
        elif action == "admin_add":
            await _handle_mod_admin_add(message, context, chat.id, rest)
        elif action == "admin_del":
            await _handle_mod_admin_del(message, context, chat.id, rest)
        elif action == "admins_list":
            await _handle_mod_admins_list(message, chat.id)
        elif action == "punished_list":
            await _handle_mod_punished_list(message, chat.id)
    except ApplicationHandlerStop:
        raise
    except Exception:
        log.exception("Ошибка команды чат-модерации")
        stats["errors"] += 1
...
def chatmod_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔇 Текст мута", callback_data="mod_mute_msg"),
         InlineKeyboardButton("🔊 Текст размута", callback_data="mod_unmute_msg")],
        [InlineKeyboardButton("🚫 Текст бана", callback_data="mod_ban_msg"),
         InlineKeyboardButton("✅ Текст разбана", callback_data="mod_unban_msg")],
        [InlineKeyboardButton("🛡 Админ назначен", callback_data="mod_admin_add_msg"),
         InlineKeyboardButton("🚷 Админ снят", callback_data="mod_admin_del_msg")],
        [InlineKeyboardButton("📋 Заголовок «Админы чата»", callback_data="mod_admins_header_msg")],
        [InlineKeyboardButton("📋 Заголовок «Наказанные»", callback_data="mod_punished_header_msg")],
        [InlineKeyboardButton("♻️ Вернуть стандартные тексты", callback_data="chatmod_reset")],
        [InlineKeyboardButton("⬅️ Админ-панель", callback_data="main")],
    ])


async def show_chatmod_menu(query) -> None:
    total_admins = sum(len(v) for v in chat_admin_ids.values())
    total_punished = sum(len(v) for v in punished.values())
    text = (
        f"🛡 {b('МОДЕРАЦИЯ ЧАТА')}\n\n"
        f"{b('помолчи')} — мут (ответом на сообщение, либо «помолчи @user 10m причина»)\n"
        f"{b('размут')} — снять мут\n"
        f"{b('гуляй')} — бан (те же варианты, срок необязателен)\n"
        f"{b('разбан')} — снять бан\n"
        f"{b('админ')} — назначить админа этого чата (ответом на сообщение) — только тебе\n"
        f"{b('разадмин')} — снять админа этого чата — только тебе\n"
        f"{b('админы')} / «список админов» — список админов чата\n"
        f"{b('наказанные')} — список замьюченных и забаненных в чате\n\n"
        "Срок задаётся так: <code>10m</code>, <code>2h</code>, <code>3d</code> или «навсегда» "
        "(по умолчанию, если не указан). Мут/бан/размут/разбан и оба списка доступны "
        "добавленным админам бота (/botadmin) и назначенным админам конкретного чата — у них "
        "нет доступа к админ-панели, только к этим командам прямо в чате.\n\n"
        f"👮 Назначено админов чата: {total_admins}\n"
        f"🚫 Сейчас наказано: {total_punished}\n\n"
        "Тексты ответов бота ниже — с жирным шрифтом и Premium Emoji; подстановки: "
        "<code>{user}</code> (упоминание), <code>{duration}</code>, <code>{reason}</code>."
    )
    await query.edit_message_text(
        text, reply_markup=chatmod_menu_keyboard(), parse_mode=ParseMode.HTML,
    )
...
async def _react_setup_report(bot) -> List[str]:
    ...
    for ch in post_channels:
        ...
        try:
            ...
            lines.append(f"❌ {title}: не удалось прочитать канал ({esc(e)})")
            continue
        if not linked:
            lines.append(f"❌ {title}: к каналу не привязана группа обсуждения")
            continue
        problems = []
        ...
        if problems:
            lines.append(f"❌ {title} → группа <code>{linked}</code>: " + "; ".join(problems))
        else:
            lines.append(f"✅ {title} → группа <code>{linked}</code>: всё настроено")
    return lines
...
async def admin_content_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get("waiting_mod_mute_msg"):
        await _save_message_setting(message, context, "waiting_mod_mute_msg", "mod_mute_text", "mod_mute_photo", "mod_mute_entities", "Текст мута")
        raise ApplicationHandlerStop

    if context.user_data.get("waiting_mod_unmute_msg"):
        await _save_message_setting(message, context, "waiting_mod_unmute_msg", "mod_unmute_text", "mod_unmute_photo", "mod_unmute_entities", "Текст размута")
        raise ApplicationHandlerStop

    if context.user_data.get("waiting_mod_ban_msg"):
        await _save_message_setting(message, context, "waiting_mod_ban_msg", "mod_ban_text", "mod_ban_photo", "mod_ban_entities", "Текст бана")
        raise ApplicationHandlerStop

    if context.user_data.get("waiting_mod_unban_msg"):
        await _save_message_setting(message, context, "waiting_mod_unban_msg", "mod_unban_text", "mod_unban_photo", "mod_unban_entities", "Текст разбана")
        raise ApplicationHandlerStop

    if context.user_data.get("waiting_mod_admin_add_msg"):
        await _save_message_setting(message, context, "waiting_mod_admin_add_msg", "mod_admin_add_text", "mod_admin_add_photo", "mod_admin_add_entities", "Текст «админ назначен»")
        raise ApplicationHandlerStop

    if context.user_data.get("waiting_mod_admin_del_msg"):
        await _save_message_setting(message, context, "waiting_mod_admin_del_msg", "mod_admin_del_text", "mod_admin_del_photo", "mod_admin_del_entities", "Текст «админ снят»")
        raise ApplicationHandlerStop

    if context.user_data.get("waiting_mod_admins_header_msg"):
        await _save_message_setting(message, context, "waiting_mod_admins_header_msg", "mod_admins_header_text", "mod_admins_header_photo", "mod_admins_header_entities", "Заголовок списка админов")
        raise ApplicationHandlerStop

    if context.user_data.get("waiting_mod_punished_header_msg"):
        await _save_message_setting(message, context, "waiting_mod_punished_header_msg", "mod_punished_header_text", "mod_punished_header_photo", "mod_punished_header_entities", "Заголовок списка наказанных")
        raise ApplicationHandlerStop
    ...
    # ---------- ПРОФИЛЬ: СТРОКА «В ЧАТЕ С» ----------
    if context.user_data.get("waiting_profile_joined"):
        if not message.text:
            await message.reply_text("❌ Отправь текст (без фото).")
            raise ApplicationHandlerStop
        text, entities = collect_entities(message)
        if "{joined}" not in text:
            await message.reply_text("❌ В тексте должна быть подстановка {joined} — туда встанет дата.")
            raise ApplicationHandlerStop
        if len(text) > 300:
            await message.reply_text("❌ Максимум 300 символов.")
            raise ApplicationHandlerStop
        S["profile_joined_text"] = text
        S["profile_joined_entities"] = entities
        await _db_set("profile_joined_text", text)
        await _db_set_entities("profile_joined_entities", entities)
        context.user_data.pop("waiting_profile_joined", None)
        preview, prev_ents = render_template(
            text, entities, {"joined": _fmt_join_date(time.time())}
        )
        await message.reply_text("✅ Строка сохранена. Предпросмотр:")
        await message.reply_text(preview, entities=prev_ents or None)
        raise ApplicationHandlerStop
    ...
    if context.user_data.get("waiting_profile_status_value"):
        ...
        # Статус, заданный админом, заменяет тот, что пользователь поставил себе сам.
        own = profile_users.get(uid)
        if own:
            own["status"] = None
            own["status_entities"] = ""
            await _save_profiles()
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Пользователь нажал «Настройки профиля → Загрузить фото» и прислал картинку.
    if message.photo and await _profile_photo_receive(message, user, chat):
        return
    if message.text and await _profile_status_receive(message, user, chat):
        return
...
async def post_init(application) -> None:
    ...
    _spawn(profile_xp_loop())
    log.info("Бот инициализирован")


async def post_shutdown(application) -> None:
    """Аккуратное завершение: каждый шаг защищён отдельно, чтобы сбой одного
    (например, MTProto) не мешал сохранить остальное и закрыть БД."""
    for task in list(_tasks):
        task.cancel()
    try:
        await asyncio.gather(*_tasks, return_exceptions=True)
    except Exception:
        log.exception("Ошибка при остановке фоновых задач")

    async def _step(name: str, coro) -> None:
        try:
            await coro
        except Exception:
            log.exception("Завершение: шаг «%s» не выполнен", name)

    await _step("MTProto-аккаунт", gift_account.close())

    if db.is_ready():
        await _step("известные пользователи", _save_known_users())
        await _step("бонусы за реакции", _save_react_state())
        await _step("статистика", _flush_stats())
    else:
        log.warning(
            "Бот остановили до окончания запуска (хранилище не готово) — "
            "сохранение состояния пропущено, чтобы не затереть данные."
        )
    await _step("закрытие БД", db.close_db())
...
def main() -> None:
    ...
    # --- помолчи / гуляй / размут / разбан / админ / разадмин / админы / наказанные
    #     (отдельная группа, раньше access_guard — проверяет права и чат сам) ---
    app.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, chat_mod_handler), group=-6
    )
    ...
    app.add_handler(CallbackQueryHandler(profile_settings_callback, pattern=r"^pset:"))
    ...
    # --- время вступления в чат (для строки «в чате с» в профиле) ---
    app.add_handler(
        MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, profile_join_handler),
        group=4,
    )
