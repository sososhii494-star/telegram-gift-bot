...
async def show_user_profile(message, user) -> None:
    ...
    try:
        if S.get("profile_photo"):
            await message.reply_photo(photo=S["profile_photo"], caption=text, caption_entities=entities or None)
        else:
...
def main() -> None:
    ...
    app.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, vip_info_handler, group=-4)
    )
