...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data in ("profile_levelup_text", "profile_bear_text", "profile_inventory_empty_text", "profile_chat_xp_text", "profile_withdraw_success_text", "profile_withdraw_fail_text"):
        _clear_waiting(context)
        mapping = {
...
async def show_profile_menu(query, note: str = "") -> None:
    ...
    text = (
        ...
        "Если у пользователя есть 🧸, в профиле появляется кнопка «Вывести мишку»."
    )
...
def _profile_inventory_text(rec: Dict[str, Any]) -> str:
    items = [x for x in rec.get("inventory", []) if isinstance(x, dict) and not x.get("withdrawn")]
    if not items:
...
def _profile_values(user) -> Dict[str, str]:
    ...
    return {
        ...
        "status": str(profile_statuses.get(user.id) or S.get("profile_status_text") or "Активный участник"),
    }
...
async def show_user_profile(message, user) -> None:
    ...
    if _profile_inventory_count(rec, "bear") > 0:
        buttons.append([InlineKeyboardButton("🧸 Вывести мишку", callback_data="inv:withdraw_bear")])
    reply_markup = InlineKeyboardMarkup(buttons) if buttons else None
    try:
        if S.get("profile_photo"):
            await message.reply_photo(photo=S["profile_photo"], caption=text, caption_entities=entities or None, reply_markup=reply_markup)
        else:
