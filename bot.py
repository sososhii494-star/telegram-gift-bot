...
DEFAULT_PROFILE_INVENTORY_EMPTY_TEXT = "🎒 Инвентарь пуст."
DEFAULT_PROFILE_CHAT_XP_TEXT = (
    "⚡ {chat} собрал XP!\n\n"
    "👥 Участников: {users}\n"
    "✨ Каждому начислено: {xp_min}–{xp_max} XP\n"
    "🔥 Всего начислено: {total_xp} XP"
)
DEFAULT_PROFILE_SETTINGS_TEXT = (
    "⚙️ Настройки профиля\n\n"
    "Здесь ты можешь изменить фото профиля и свой статус.\n"
    "Обычный статус поддерживает обычные эмодзи.\n"
    "💎 VIP-пользователям разрешены Premium Emoji в статусе."
)
...
DEFAULT_PROFILE_INVENTORY_ITEM_TEXT = "{name} ×{count}"
...
async def inventory_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    data = query.data or ""
    if data == "inv:noop":
        await query.answer("🎒 Инвентарь уже обновлён.")
        return
    if data not in ("inv:withdraw_bear", "inv:withdraw"):
        return
    if data == "inv:withdraw":
        context.user_data["profile_withdraw_user"] = True
        await query.answer()
        await query.edit_message_text(str(S.get("profile_withdraw_ask_text") or DEFAULT_PROFILE_WITHDRAW_ASK_TEXT), reply_markup=back_kb("prof:inventory"))
        return
    await query.answer("⏳ Вывод мишки...", show_alert=False)
    user = query.from_user
    rec = _profile_record(user)
    bear = next((x for x in rec.get("inventory", []) if isinstance(x, dict) and x.get("type") == "bear" and not x.get("withdrawn")), None)
    if not bear:
        await query.answer("❌ Мишки нет в инвентаре.", show_alert=True)
        return
    if _PROFILE_CONTEXT is None:
        await query.answer("❌ Бот ещё не готов.", show_alert=True)
        return
    try:
        gift = await ga_find_bear_gift()
        if not gift:
            raise RuntimeError("Не найден Telegram-подарок 🧸. Укажи ID подарка в админ-панели или проверь привязанный аккаунт.")
        await ga_send_gift(user.id, int(gift["id"]), "🧸 Награда за 2 уровень профиля")
        bear["withdrawn"] = True
        bear["withdrawn_at"] = time.time()
        bear["telegram_gift_id"] = int(gift["id"])
        bear["stars"] = int(gift.get("stars", 0) or 0)
        await _save_profiles()
        values = {"name": str(user.full_name or "Без имени"), "username": f"@{user.username}" if user.username else str(user.id), "stars": str(gift.get("stars", 0) or 0)}
        text, entities = render_template(str(S.get("profile_withdraw_success_text") or DEFAULT_PROFILE_WITHDRAW_SUCCESS_TEXT), S.get("profile_withdraw_success_entities") or [], values)
        await context.bot.send_message(chat_id=query.message.chat_id, text=text, entities=entities or None)
        try:
            await query.message.edit_reply_markup(reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎒 Инвентарь обновлён", callback_data="inv:noop")]]))
        except TelegramError:
            pass
    except Exception as e:
        log.exception("Не удалось вывести мишку пользователю %s", user.id)
        values = {"name": str(user.full_name or "Без имени"), "username": f"@{user.username}" if user.username else str(user.id), "error": str(e)[:500]}
        text, entities = render_template(str(S.get("profile_withdraw_fail_text") or DEFAULT_PROFILE_WITHDRAW_FAIL_TEXT), S.get("profile_withdraw_fail_entities") or [], values)
        await context.bot.send_message(chat_id=query.message.chat_id, text=text, entities=entities or None)
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if len(parts) >= 3:
            ...
            if action in ("addxp", "setxp", "setlevel", "giveitem"):
                ...
                prompts = {
                    ...
                    "giveitem": "Отправь название предмета. Для мишки используй <code>bear</code> или <code>мишка</code>.",
                }
                await query.edit_message_text(
            ...
            if action == "clearinv":
                ...
                rec["inventory"] = []
                await _save_profiles()
                await show_profile_user_admin_menu(query, uid, "✅ Инвентарь очищен.")
                return

    if data in ("profile_levelup_text", "profile_bear_text", "profile_inventory_empty_text", "profile_chat_xp_text", "profile_withdraw_success_text", "profile_withdraw_fail_text", "profile_settings_text", "profile_status_saved_text", "profile_photo_saved_text", "profile_status_reset_text", "profile_photo_removed_text", "profile_inventory_text", "profile_withdraw_ask_text", "profile_withdraw_busy_text", "profile_withdraw_no_item_text", "profile_withdraw_invalid_recipient_text"):
        _clear_waiting(context)
        mapping = {
            ...
            "profile_settings_text": ("waiting_profile_settings_text", "profile_settings_text", None, None, "Текст настроек профиля"),
            "profile_status_saved_text": ("waiting_profile_status_saved_text", "profile_status_saved_text", None, None, "Текст сохранения статуса"),
            "profile_photo_saved_text": ("waiting_profile_photo_saved_text", "profile_photo_saved_text", None, None, "Текст сохранения фото"),
            "profile_status_reset_text": ("waiting_profile_status_reset_text", "profile_status_reset_text", None, None, "Текст сброса статуса"),
            "profile_photo_removed_text": ("waiting_profile_photo_removed_text", "profile_photo_removed_text", None, None, "Текст удаления фото"),
            "profile_inventory_text": ("waiting_profile_inventory_text", "profile_inventory_text", None, None, "Текст экрана инвентаря"),
            "profile_withdraw_ask_text": ("waiting_profile_withdraw_ask_text", "profile_withdraw_ask_text", None, None, "Текст запроса получателя"),
            "profile_withdraw_busy_text": ("waiting_profile_withdraw_busy_text", "profile_withdraw_busy_text", None, None, "Текст процесса вывода"),
            "profile_withdraw_no_item_text": ("waiting_profile_withdraw_no_item_text", "profile_withdraw_no_item_text", None, None, "Текст отсутствия приза"),
            "profile_withdraw_invalid_recipient_text": ("waiting_profile_withdraw_invalid_recipient_text", "profile_withdraw_invalid_recipient_text", None, None, "Текст неверного получателя"),
        }
        ...
        await query.edit_message_text(prompt, reply_markup=back_kb("profile_texts"), parse_mode=ParseMode.HTML)
        return
    ...
    if data == "profile_message":
        ...
        await query.edit_message_text(
            ...
            "<code>{remaining_xp}</code> — сколько XP осталось\n"
            "<code>{gifts}</code> — полученные подарки\n"
            "<code>{inventory}</code> — содержимое инвентаря; его можно выделить жирным/Premium Emoji прямо в этом шаблоне\n"
...
def profile_texts_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⚡ Текст сбора XP по команде хп", callback_data="profile_chat_xp_text")],
        ...
        [InlineKeyboardButton("🎒 Текст инвентаря", callback_data="profile_inventory_text")],
...
async def show_profile_texts_menu(query, note: str = "") -> None:
    await query.edit_message_text(
        ...
        "Подстановки уровня: <code>{username}</code> <code>{level}</code> <code>{xp}</code> <code>{reward}</code>.\n"
        "Для сбора <code>хп</code>: <code>{chat}</code> <code>{users}</code> <code>{xp_min}</code> <code>{xp_max}</code> <code>{total_xp}</code>.",
...
def _profile_user_admin_kb(uid: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🧸 Выдать мишку", callback_data=f"profile_user:giveitem:{uid}"), InlineKeyboardButton("🗑 Очистить инвентарь", callback_data=f"profile_user:clearinv:{uid}")],
        [InlineKeyboardButton("⬅️ Профиль", callback_data="profile_menu")],
    ])
...
async def show_profile_user_admin_menu(query, uid: int, note: str = "") -> None:
    ...
    inv = _profile_inventory_text(rec)
    await query.edit_message_text(
...
async def show_profile_menu(query, note: str = "") -> None:
    ...
    text = (
        ...
        "⚡ Автоматически: каждые 60 минут +3–7 XP активным пользователям.\n"
        "📣 Команда <code>хп</code>: раз в час начисляет +3–7 XP всем участникам, которых бот видел в этом чате.\n\n"
...
def _profile_inventory_count(rec: Dict[str, Any], item_type: str) -> int:
    return sum(1 for item in rec.get("inventory", []) if isinstance(item, dict) and item.get("type") == item_type and not item.get("withdrawn"))
...
def _profile_inventory_text(rec: Dict[str, Any]) -> str:
    items = _profile_inventory_items(rec)
    if not items:
        return str(S.get("profile_inventory_empty_text") or DEFAULT_PROFILE_INVENTORY_EMPTY_TEXT)
    counts: Dict[str, int] = {}
    for item in items:
        name = str(item.get("name") or item.get("type") or "Предмет")
        counts[name] = counts.get(name, 0) + 1
    template = str(S.get("profile_inventory_item_text") or DEFAULT_PROFILE_INVENTORY_ITEM_TEXT)
    return "\n".join(template.format(name=name, count=count) for name, count in counts.items())
...
def _profile_values(user) -> Dict[str, str]:
    ...
    return {
        ...
        "inventory": _profile_inventory_text(rec),
...
def _render_profile(user) -> Tuple[str, List[MessageEntity]]:
    ...
    # Если инвентарь пуст, его отдельное оформление из админки тоже переносим
    # в итоговое сообщение профиля (включая Premium Emoji).
    ...
    if status_value and status_entities:
        idx = text.find(status_value)
        if idx >= 0:
            base = _u16(text[:idx])
            for entity in status_entities:
                entities.append(_clone_entity_len(entity, base + int(entity.offset), int(entity.length)))

    inv_value = str(values.get("inventory", ""))
    if inv_value and inv_value == str(S.get("profile_inventory_empty_text") or DEFAULT_PROFILE_INVENTORY_EMPTY_TEXT):
        idx = text.find(inv_value)
        if idx >= 0 and S.get("profile_inventory_empty_entities"):
            base = _u16(text[:idx])
            for entity in S.get("profile_inventory_empty_entities") or []:
                entities.append(_clone_entity_len(entity, base + int(entity.offset), int(entity.length)))
    return text, entities
...
async def _announce_profile_level(chat_id: int, user, old_level: int, new_level: int, xp: int, context: Optional[ContextTypes.DEFAULT_TYPE] = None) -> None:
    ...
    values = {
        ...
        "level": str(new_level),
        "xp": str(xp),
        "reward": "🧸 Мишка" if old_level < 2 <= new_level else "—",
    }
    ...
    try:
        if S.get("profile_levelup_photo"):
            await context.bot.send_photo(chat_id=chat_id, photo=S["profile_levelup_photo"], caption=text, caption_entities=entities or None)
        else:
...
async def _award_level_rewards(user_id: int, user, old_level: int, new_level: int, chat_id: int, context) -> None:
    if old_level < 2 <= new_level:
        rec = _profile_record(user)
        if not any(isinstance(x, dict) and x.get("type") == "bear" and not x.get("withdrawn") for x in rec.get("inventory", [])):
            _profile_add_inventory_item(user_id, "bear", "🧸 Мишка", "level_2")
            values = {"name": str(getattr(user, "full_name", None) or "Без имени"), "username": f"@{user.username}" if user.username else str(user.id), "level": "2", "xp": str(rec.get("xp", 0)), "reward": "🧸 Мишка"}
            ...
            try:
                if S.get("profile_bear_photo"):
                    await context.bot.send_photo(chat_id=chat_id, photo=S["profile_bear_photo"], caption=text, caption_entities=entities or None)
                else:
                    await context.bot.send_message(chat_id=chat_id, text=text, entities=entities or None)
            except TelegramError:
...
async def _profile_apply_xp(user_id: int, amount: int, chat_id: int, context, reason="") -> Dict[str, Any]:
    ...
    user = user or _profile_user_object(int(user_id), rec)
...
async def collect_chat_xp(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not message or not chat or chat.type not in ("group", "supergroup"):
        return
    now_ts = time.time()
    last = float(profile_chat_xp_claims.get(int(chat.id), 0) or 0)
    if now_ts - last < PROFILE_XP_INTERVAL_SECONDS:
        remaining = int(PROFILE_XP_INTERVAL_SECONDS - (now_ts - last))
        mins = max(1, remaining // 60)
        await message.reply_text(f"⏳ XP в этом чате уже собирали. Следующий сбор примерно через {mins} мин.")
        return

    members = []
    for uid, rec in list(profile_users.items()):
        if int(chat.id) not in [int(x) for x in rec.get("chat_ids", [])]:
            continue
        if uid not in known_users and not rec.get("name") and not rec.get("username"):
            continue
        members.append((int(uid), rec))
    if caller and all(uid != caller.id for uid, _ in members):
        rec = _profile_record(caller)
        await _profile_touch_chat(caller, chat.id)
        members.append((caller.id, rec))

    if not members:
        profile_chat_xp_claims[int(chat.id)] = now_ts
        await _save_profiles()
        await message.reply_text("ℹ️ Пока нет участников, которых бот видел в этом чате.")
        return

    profile_chat_xp_claims[int(chat.id)] = now_ts
    total = 0
    for uid, rec in members:
        amount = random.randint(PROFILE_XP_MIN, PROFILE_XP_MAX)
        total += amount
        await _profile_apply_xp(uid, amount, int(chat.id), context, reason="chat_claim")

    values = {
        "chat": str(getattr(chat, "title", None) or "Чат"),
        "users": str(len(members)),
        "xp_min": str(PROFILE_XP_MIN),
        "xp_max": str(PROFILE_XP_MAX),
        "total_xp": str(total),
    }
    text, entities = render_template(str(S.get("profile_chat_xp_text") or DEFAULT_PROFILE_CHAT_XP_TEXT), S.get("profile_chat_xp_entities") or [], values)
    await message.reply_text(text, entities=entities or None)
    await _save_profiles()


async def profile_xp_loop() -> None:
    """Раз в час активным пользователям выдаёт случайные 3–7 XP и награды за уровни."""
    while True:
        try:
        ...
        for uid, rec in list(profile_users.items()):
            if now - float(rec.get("last_activity", 0)) > 2 * 3600:
                continue
            if now - float(rec.get("last_xp_at", 0) or 0) < PROFILE_XP_INTERVAL_SECONDS:
                continue
            old_xp = max(0, int(rec.get("xp", 0)))
            old_level = _profile_level(old_xp)
            rec["xp"] = old_xp + random.randint(PROFILE_XP_MIN, PROFILE_XP_MAX)
            rec["last_xp_hour"] = current_hour
            rec["last_xp_at"] = now
            changed = True
            new_level = _profile_level(int(rec["xp"]))
            if new_level > old_level and _PROFILE_CONTEXT is not None:
                try:
                    user_obj = _profile_user_object(int(uid), rec)
                    chat_id = int(rec.get("last_chat_id", 0) or 0)
                    if chat_id:
                        await _announce_profile_level(chat_id, user_obj, old_level, new_level, int(rec["xp"]), _PROFILE_CONTEXT)
                        await _award_level_rewards(int(uid), user_obj, old_level, new_level, chat_id, _PROFILE_CONTEXT)
                except Exception:
                    log.exception("Не удалось выдать награду за уровень пользователю %s", uid)
        if changed:
            await _save_profiles()


def profile_user_keyboard(rec: Dict[str, Any]) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton("🎒 Инвентарь", callback_data="prof:inventory")],
        [InlineKeyboardButton("⚙️ Настройки профиля", callback_data="prof:settings")],
    ]
    if _profile_inventory_count(rec, "bear") > 0:
        rows.insert(1, [InlineKeyboardButton("🎁 Вывести приз", callback_data="inv:withdraw")])
    return InlineKeyboardMarkup(rows)

def _profile_settings_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📷 Изменить фото", callback_data="prof:photo")],
        [InlineKeyboardButton("📝 Изменить статус", callback_data="prof:status")],
        [InlineKeyboardButton("🗑 Удалить фото", callback_data="prof:remove_photo")],
        [InlineKeyboardButton("⬅️ Профиль", callback_data="prof:back")],
    ])

async def show_user_inventory(target_message, user) -> None:
    rec = _profile_record(user)
    values = {
        "username": f"@{user.username}" if user.username else user.full_name or str(user.id),
        "items": _profile_inventory_text(rec),
    }
    text = str(S.get("profile_inventory_text") or DEFAULT_PROFILE_INVENTORY_TEXT).format(**values)
    buttons = []
    if _profile_inventory_count(rec, "bear") > 0:
        buttons.append([InlineKeyboardButton("🎁 Вывести приз", callback_data="inv:withdraw")])
    buttons.append([InlineKeyboardButton("⚙️ Настройки профиля", callback_data="prof:settings")])
    buttons.append([InlineKeyboardButton("⬅️ Профиль", callback_data="prof:back")])
    await target_message.reply_text(text, reply_markup=InlineKeyboardMarkup(buttons))
...
async def profile_user_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "prof:back":
        await query.answer()
        await show_user_profile(query.message, user)
        return
    ...
    if data == "prof:settings":
        await query.answer()
        await query.edit_message_text(str(S.get("profile_settings_text") or DEFAULT_PROFILE_SETTINGS_TEXT), reply_markup=_profile_settings_keyboard())
        return
    if data == "prof:photo":
        await query.answer()
        context.user_data["profile_wait_photo"] = True
        await query.edit_message_text("📷 Отправь сюда свою фотографию одним сообщением.\n\n/cancel — отмена.", reply_markup=back_kb("prof:settings"))
        return
    if data == "prof:remove_photo":
        rec["profile_photo"] = ""
        _schedule_profile_save()
        await query.answer("Фото удалено.")
        await query.edit_message_text(str(S.get("profile_photo_removed_text") or DEFAULT_PROFILE_PHOTO_REMOVED_TEXT), reply_markup=_profile_settings_keyboard())
        return
    if data == "prof:status":
        await query.answer()
        context.user_data["profile_wait_status"] = True
        await query.edit_message_text("📝 Отправь новый статус одним сообщением.\nДо 120 символов. Обычный эмодзи доступен всем.\n💎 Premium Emoji сохраняются только для VIP.\n\n/cancel — отмена.", reply_markup=back_kb("prof:settings"))
        return
...
async def show_user_profile(message, user) -> None:
    ...
    reply_markup = profile_user_keyboard(rec)
    photo = rec.get("profile_photo") or None
    try:
        if photo:
            await message.reply_photo(photo=photo, caption=text, caption_entities=entities or None, reply_markup=reply_markup)
        else:
            await message.reply_text(text, entities=entities or None, reply_markup=reply_markup)
    except TelegramError:
...
async def admin_content_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user and message:
        if context.user_data.get("profile_wait_photo"):
            ...
            _schedule_profile_save()
            await message.reply_text(str(S.get("profile_photo_saved_text") or DEFAULT_PROFILE_PHOTO_SAVED_TEXT), reply_markup=_profile_settings_keyboard())
            raise ApplicationHandlerStop
        if context.user_data.get("profile_wait_status"):
            ...
            if status_text.strip().lower() in ("сброс", "reset", "off", "-"):
                ...
                _schedule_profile_save()
                await message.reply_text(str(S.get("profile_status_reset_text") or DEFAULT_PROFILE_STATUS_RESET_TEXT), reply_markup=_profile_settings_keyboard())
                raise ApplicationHandlerStop
            if not _profile_is_vip(user):
                status_entities = [e for e in status_entities if getattr(e, "type", "") != MessageEntity.CUSTOM_EMOJI]
            ...
            context.user_data.pop("profile_wait_status", None)
            _schedule_profile_save()
            await message.reply_text(str(S.get("profile_status_saved_text") or DEFAULT_PROFILE_STATUS_SAVED_TEXT).format(status=status_text))
            raise ApplicationHandlerStop
        if context.user_data.get("profile_withdraw_user"):
            raw = (message.text or "").strip()
            if not raw or len(raw) > 100:
                await message.reply_text(str(S.get("profile_withdraw_invalid_recipient_text") or DEFAULT_PROFILE_WITHDRAW_INVALID_RECIPIENT_TEXT))
                raise ApplicationHandlerStop
            rec = _profile_record(user)
            bear = next((x for x in rec.get("inventory", []) if isinstance(x, dict) and x.get("type") == "bear" and not x.get("withdrawn")), None)
            if not bear:
                context.user_data.pop("profile_withdraw_user", None)
                await message.reply_text(str(S.get("profile_withdraw_no_item_text") or DEFAULT_PROFILE_WITHDRAW_NO_ITEM_TEXT))
                raise ApplicationHandlerStop
            recipient = raw
            context.user_data.pop("profile_withdraw_user", None)
            await message.reply_text(str(S.get("profile_withdraw_busy_text") or DEFAULT_PROFILE_WITHDRAW_BUSY_TEXT))
            try:
                gift = await ga_find_bear_gift()
                if not gift:
                    raise RuntimeError("Не найден Telegram-подарок 🧸. Проверь привязанный аккаунт и ID подарка.")
                await ga_send_gift(recipient, int(gift["id"]), "🧸 Награда за 2 уровень профиля")
                bear["withdrawn"] = True
                bear["withdrawn_at"] = time.time()
                bear["withdraw_to"] = recipient
                bear["telegram_gift_id"] = int(gift["id"])
                bear["stars"] = int(gift.get("stars", 0) or 0)
                await _save_profiles()
                values = {"name": user.full_name or "Без имени", "username": f"@{user.username}" if user.username else str(user.id), "stars": str(gift.get("stars", 0) or 0)}
                text, entities = render_template(str(S.get("profile_withdraw_success_text") or DEFAULT_PROFILE_WITHDRAW_SUCCESS_TEXT), S.get("profile_withdraw_success_entities") or [], values)
                await message.reply_text(text, entities=entities or None)
            except Exception as e:
                log.exception("Вывод приза для %s не удался", user.id)
                values = {"name": user.full_name or "Без имени", "username": f"@{user.username}" if user.username else str(user.id), "error": str(e)[:500]}
                text, entities = render_template(str(S.get("profile_withdraw_fail_text") or DEFAULT_PROFILE_WITHDRAW_FAIL_TEXT), S.get("profile_withdraw_fail_entities") or [], values)
                await message.reply_text(text, entities=entities or None)
            raise ApplicationHandlerStop
    ...
    if context.user_data.get("waiting_profile_manage_user"):
        ...
        if uid is None:
            ...
            matches = [(known_id, info) for known_id, info in known_users.items() if info.get("u") == uname]
            if len(matches) == 1:
                uid = int(matches[0][0])
            else:
                await message.reply_text("❌ Не нашёл пользователя. Укажи числовой ID или @username человека, которого бот уже видел в чате.")
                raise ApplicationHandlerStop
        ...
        await message.reply_text(
            ...
            f"🎒 Инвентарь: {esc(_profile_inventory_text(rec))}",
            reply_markup=_profile_user_admin_kb(uid), parse_mode=ParseMode.HTML,
        )
        raise ApplicationHandlerStop
        ...
        if context.user_data.get(flag):
            ...
            try:
                if action == "addxp":
                    ...
                    item_name = "🧸 Мишка" if raw.lower() in ("bear", "мишка", "🧸") else raw[:100]
                    if not item_name:
                        raise ValueError
                    _profile_add_inventory_item(uid, "bear" if item_name == "🧸 Мишка" else "custom", item_name, "admin")
                    await _save_profiles()
    ...
    text_settings = [
        ...
        ("waiting_profile_settings_text", "profile_settings_text", None, None, "Текст настроек профиля"),
        ("waiting_profile_status_saved_text", "profile_status_saved_text", None, None, "Текст сохранения статуса"),
        ("waiting_profile_photo_saved_text", "profile_photo_saved_text", None, None, "Текст сохранения фото"),
        ("waiting_profile_status_reset_text", "profile_status_reset_text", None, None, "Текст сброса статуса"),
        ("waiting_profile_photo_removed_text", "profile_photo_removed_text", None, None, "Текст удаления фото"),
        ("waiting_profile_inventory_text", "profile_inventory_text", None, None, "Текст экрана инвентаря"),
        ("waiting_profile_withdraw_ask_text", "profile_withdraw_ask_text", None, None, "Текст запроса получателя"),
        ("waiting_profile_withdraw_busy_text", "profile_withdraw_busy_text", None, None, "Текст процесса вывода"),
        ("waiting_profile_withdraw_no_item_text", "profile_withdraw_no_item_text", None, None, "Текст отсутствия приза"),
        ("waiting_profile_withdraw_invalid_recipient_text", "profile_withdraw_invalid_recipient_text", None, None, "Текст неверного получателя"),
    ]
