...
def _pset_menu_kb(uid: int, has_photo: bool) -> InlineKeyboardMarkup:
    rows = [[InlineKeyboardButton("🖼 Загрузить своё фото", callback_data=f"pset:photo:{uid}")]]
    if has_photo:
...
async def show_user_profile(message, user) -> None:
    ...
    try:
        ...
        if custom:
            try:
                await message.reply_photo(
                    photo=custom, caption=text, caption_entities=entities or None, reply_markup=kb
                )
                return
            except BadRequest:
        ...
        if S.get("profile_photo"):
            await message.reply_photo(
                photo=S["profile_photo"], caption=text, caption_entities=entities or None, reply_markup=kb
            )
        else:
            await message.reply_text(text, entities=entities or None, reply_markup=kb)
    except TelegramError:
...
async def profile_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "menu":
        await query.answer()
        await _set_kb(_pset_menu_kb(user.id, bool(rec.get("photo"))))
    elif action == "back":
        await query.answer()
