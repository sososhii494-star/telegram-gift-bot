...
# Пользователи, которым запрещена выдача подарков.
# Хранятся без символа @ и в нижнем регистре.
...
def is_blocked_user(update):
    user = update.effective_user
    if not user or not user.username:
        return False
    return normalize_username(user.username) in blocked_usernames
...
async def admin_callback(
    ...
    if data == "blocked":
        await query.edit_message_text(
            "🚫 **БЛОКИРОВКА ВЫДАЧ**\n\n"
            f"Заблокировано: **{len(blocked_usernames)}**\n\n"
            "Добавляй username без ограничений. "
            "При выпадении победы заблокированному пользователю "
            "подарок не отправится.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ Добавить username", callback_data="blocked:add")],
                [InlineKeyboardButton("➖ Удалить username", callback_data="blocked:remove")],
                [InlineKeyboardButton("📋 Список", callback_data="blocked:list")],
                [InlineKeyboardButton("🗑 Очистить список", callback_data="blocked:clear")],
    ...
    if data == "blocked:add":
        ...
        context.user_data["waiting_blocked_remove"] = False
        await query.edit_message_text(
            "➕ **ДОБАВЛЕНИЕ В БЛОКИРОВКУ**\n\n"
            "Отправь username с @ или без него. Можно сразу несколько — "
            "через пробел или с новой строки.\n\n"
            "❌ `/cancel` — отменить.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Назад", callback_data="blocked")]]),
            parse_mode="Markdown"
        )
        return
    ...
    if data == "blocked:remove":
        ...
        context.user_data["waiting_blocked_add"] = False
        await query.edit_message_text(
            "➖ **УДАЛЕНИЕ ИЗ БЛОКИРОВКИ**\n\n"
            "Отправь username с @ или без него. Можно сразу несколько.\n\n"
            "❌ `/cancel` — отменить.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Назад", callback_data="blocked")]]),
            parse_mode="Markdown"
    ...
    if data == "blocked:list":
        if not blocked_usernames:
            list_text = "📋 Список блокировки пуст."
        else:
            names = sorted(blocked_usernames)
            list_text = "📋 **ЗАБЛОКИРОВАННЫЕ USERNAME**\n\n" + "\n".join(f"{i}. @{n}" for i, n in enumerate(names, 1))
            if len(list_text) > 4000:
                list_text = f"📋 **ЗАБЛОКИРОВАННЫЕ USERNAME**\n\nВсего: **{len(names)}**\nСписок слишком большой для одного сообщения."
        await query.edit_message_text(
            list_text,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Назад", callback_data="blocked")]]),
            parse_mode="Markdown"
    ...
    if data == "blocked:clear":
        ...
        await query.edit_message_text(
            "✅ **Список блокировки очищен.**",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🚫 Блокировка выдач", callback_data="blocked")],
...
async def admin_content_handler(
    ...
    # =====================================================
    # БЛОКИРОВКА: ДОБАВИТЬ / УДАЛИТЬ USERNAME
    # =====================================================

    if context.user_data.get("waiting_blocked_add"):

        if not message.text:
            await message.reply_text("❌ Отправь username текстом, например @username.")
            return

        import re
        names = [normalize_username(x) for x in re.split(r"[\s,;]+", message.text) if normalize_username(x)]
        ...
        if names:
            await message.reply_text(
                ...
                f"\n\nВсего в списке: **{len(blocked_usernames)}**",
                parse_mode="Markdown"
            )
        else:
            await message.reply_text("❌ Username не найден.")
    ...
    if context.user_data.get("waiting_blocked_remove"):
        ...
        import re
        names = [normalize_username(x) for x in re.split(r"[\s,;]+", message.text) if normalize_username(x)]
        removed = [n for n in names if n in blocked_usernames]
        for n in removed:
            blocked_usernames.remove(n)
        context.user_data["waiting_blocked_remove"] = False

        await message.reply_text(
            "✅ **Разблокированы:**\n" + ("\n".join(f"• @{n}" for n in removed) if removed else "Никого не найдено.") +
            f"\n\nВсего в списке: **{len(blocked_usernames)}**",
            parse_mode="Markdown"
        )
        raise ApplicationHandlerStop
...
async def message_handler(
    ...
    # -----------------------------------------------------
    # ПРОВЕРКА БЛОКИРОВКИ ВЫДАЧ
    # -----------------------------------------------------

    if is_blocked_user(update):

        if random.random() * 100 < get_chance(context):
            await update.message.reply_text(BLOCKED_WIN_TEXT)
    ...
    app.add_handler(
        ...
        CallbackQueryHandler(
            ...
            pattern=(
                ...
                r"toggle|refresh|blocked(?::(?:add|remove|list|clear))?|"
                r"setchance:.*|winmessage)$"
            )
        )
    )
