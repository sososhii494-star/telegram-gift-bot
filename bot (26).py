...
def _botadmins_text(note: str = "") -> str:
    ...
    lines += [
        ...
        f"{b(SUMRAK_ON_WORD)} / {b(SUMRAK_OFF_WORD)} и не попадают под антиспам. "
...
_SUMRAK_RE = re.compile(r"^!\s*сумрак(?:\s*(офф|off))?\s*[.!]*$", re.IGNORECASE)


async def _set_chat_closed(bot, chat_id: int, close: bool) -> None:
    """Закрывает/открывает чат без ограничения по времени (как кнопки 🔒/🔓)."""
    ...
    if close:
        # В закрытом чате обычные участники не пишут — а админы бота, если они
        # не админы самого Telegram-чата, тоже потеряли бы право написать
        # !сумракофф. Выдаём им личное разрешение писать.
        for uid in {ADMIN_ID, *bot_admin_ids.keys()}:
            try:
...
async def sumrak_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat.type in ("group", "supergroup"):
        ...
        if len(allowed_chat_ids) == 1:
            ...
            await message.reply_text(
                f"У бота несколько привязанных чатов — напиши {SUMRAK_OFF_WORD if is_off else SUMRAK_ON_WORD} "
                "прямо в нужном чате."
            )
            raise ApplicationHandlerStop
    ...
    for chat_id in targets:
        try:
            await _set_chat_closed(context.bot, chat_id, close)
        except TelegramError as e:
        ...
        text = ("🌑 Сумрак. Чат закрыт." if close else "🌕 Сумрак рассеялся. Чат открыт.")
        if chat.type in ("group", "supergroup"):
