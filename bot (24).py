...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "manual_gift_start":
        ...
        await query.edit_message_text(
            f"🎁 {b('ПОЛУЧАТЕЛЬ')}\n\n"
            "Отправь @username или числовой Telegram ID получателя.\n\n"
...
def antispam_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📝 Текст + фото + Premium Emoji", callback_data="antispam_message")],
...
async def show_antispam_menu(query) -> None:
    ...
    await query.edit_message_text(
        ...
        "также напоминает об этом правиле (0 — не напоминать).\n\n"
...
async def _send_manual_gift(recipient: str, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Ручная дополнительная выдача из админ-панели по @username или ID."""
    try:
        gifts = await _available_gifts(context)
        if not gifts:
            raise RuntimeError("Нет доступных Telegram-подарков")
        wanted = S.get("selected_gift_id")
        if wanted:
            gift = next((g for g in gifts if str(g["id"]) == str(wanted)), None)
            if gift is None:
                raise RuntimeError(f"Подарок ID {wanted} сейчас недоступен")
        else:
            gift = min(gifts, key=lambda g: g["stars"])

        await gift_account.send_gift(recipient, int(gift["id"]), None)
    except Exception as e:
        log.exception("Ручная выдача не удалась")
        await _notify_admin(context, f"❌ Ручная выдача не удалась для {recipient}: {e}")
        return False

    text = str(S.get("manual_gift_text") or DEFAULT_MANUAL_GIFT_TEXT)
    text = text.replace("{recipient}", esc(recipient))
    entities = S.get("manual_gift_entities") or None
    photo = S.get("manual_gift_photo")

    # Ищем чат, где этот человек писал последний раз (по @username или по
    # числовому ID) — сообщение/фото о выдаче уходит туда, ответом на его
    # последнее сообщение, а не в чат админа.
    info = None
    uname = _normalize_username(recipient)
    if uname:
        info = last_message_by_username.get(uname)
    if info is None:
        try:
            info = last_message_by_user_id.get(int(recipient))
        except (TypeError, ValueError):
            info = None
    ...
    # Фолбэк: если чат человека не найден (он ещё не писал в разрешённых
    # чатах) или отправка туда не удалась — как раньше, шлём в чат админа.
...
async def admin_content_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.user_data.get("waiting_manual_gift_recipient"):
        ...
        sent = await _send_manual_gift(recipient, context)
        if sent:
...
async def check_antispam(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        ...
        if count % every == 0:
            try:
                await context.bot.send_message(
                    chat_id=chat.id,
                    text=(
                        f"⚠️ {b('Напоминание')}: больше {limit} сообщений в минуту "
                        f"— мут чата на {mute_minutes} мин.\n"
                        f"Причина: {S.get('antispam_reason') or ANTISPAM_REASON}."
                    ),
                    parse_mode=ParseMode.HTML,
                )
            except TelegramError:
    ...
    try:
        text = str(S.get("antispam_text") or DEFAULT_ANTISPAM_TEXT)
        text = text.replace("{user}", user.mention_html()).replace("{minutes}", str(mute_minutes)).replace("{reason}", esc(S.get("antispam_reason") or ANTISPAM_REASON))
        if S.get("antispam_photo"):
            await message.reply_photo(photo=S["antispam_photo"], caption=text, caption_entities=S.get("antispam_entities") or None)
        else:
            await message.reply_text(text, entities=S.get("antispam_entities") or None)
    except TelegramError:
        pass
