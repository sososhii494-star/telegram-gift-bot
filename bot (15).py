...
async def load_persistent_state() -> None:
    ...
    S["guess_secret"] = await db.get_int_setting("guess_secret", 0) or None
...
def start_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton(
                f"🔒 Закрыть чат — {S['price_close_chat']} ⭐",
                callback_data="buy:close",
            )],
            [InlineKeyboardButton(
                f"🍀 Повысить шанс на подарок — {S['price_boost']} ⭐",
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        f"{b('МАГАЗИН ЗА ⭐')}\n"
        f"🔒 Закрыть чат на {fmt_num(S['close_minutes'])} мин — {b(S['price_close_chat'])} ⭐\n"
        f"🍀 Шанс ×{fmt_num(S['boost_multiplier'])} на {fmt_num(S['boost_hours'])} ч — {b(S['price_boost'])} ⭐\n\n"
...
def guess_menu_keyboard() -> InlineKeyboardMarkup:
    toggle_text = "⛔ Остановить" if S["guess_enabled"] else "🚀 Запустить"
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton(
                toggle_text,
            ...
            [InlineKeyboardButton("🎁 Приз победителю", callback_data="guess_prize")],
            [InlineKeyboardButton("⬅️ Админ-панель", callback_data="main")],
        ]
    )
...
async def show_guess_menu(query) -> None:
    ...
    chat_line = f"\n💬 Чат: <code>{S['guess_chat_id']}</code>" if S["guess_chat_id"] else ""
    await query.edit_message_text(
        f"🔢 {b('УГАДАЙ ЧИСЛО')}\n\n"
        f"Статус: {status}{chat_line}\n"
        f"🔽 Диапазон: {b(S['guess_min'])} – {b(S['guess_max'])}\n"
        f"🎁 Приз: {esc(S['guess_prize_text'])[:200]}\n\n"
        "Запуск отправляет объявление в текущий чат (или в чат последнего "
        "запуска), закрепляет его и уведомляет участников. Первый, кто в "
        "чате напишет загаданное число, получает приз автоматически.",
...
async def launch_guess_event(query, context) -> None:
    ...
    if not chat_id:
        await query.answer(
            "❌ Открой /admin прямо в нужной группе, чтобы запустить ивент там.",
            show_alert=True,
        )
        return
    ...
    try:
        ...
        log.exception("Ошибка запуска ивента «Угадай число»")
        await query.edit_message_text(
...
async def guess_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    S["guess_chat_id"] = update.effective_chat.id
    await _db_set("guess_chat_id", S["guess_chat_id"])
    ...
    try:
        if S["guess_name_photo"]:
            sent = await update.message.reply_photo(
                photo=S["guess_name_photo"], caption=text, caption_entities=entities or None,
            )
        else:
            sent = await update.message.reply_text(text, entities=entities or None)
    except Exception:
        log.exception("Ошибка публикации ивента «Угадай число»")
        await update.message.reply_text("❌ Не удалось опубликовать сообщение ивента.")
        return
    ...
    try:
        await context.bot.pin_chat_message(
            chat_id=update.effective_chat.id, message_id=sent.message_id, disable_notification=False,
        )
    except TelegramError:
        log.warning("Не удалось закрепить сообщение ивента (нет прав?)")
